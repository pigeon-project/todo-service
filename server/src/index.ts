...
function roleFor(boardId: string, userId: string): 'admin' | 'writer' | 'reader' | null {
  const owner = db.prepare('SELECT owner FROM boards WHERE id = ?').get(boardId) as { owner?: string } | undefined;
  if (!owner) return null;
  if (owner.owner === userId) return 'admin';
  const mem = db
    .prepare('SELECT role, status FROM memberships WHERE board_id = ? AND user_id = ?')
    .get(boardId, userId) as { role: 'admin' | 'writer' | 'reader'; status: 'active' | 'pending' } | undefined;
  if (!mem) return null;
  if (mem.status !== 'active' && mem.role !== 'admin') return null; // only active non-admins count
  return mem.role;
}

function ensureRole(boardId: string, userId: string, allowed: ('admin' | 'writer' | 'reader')[], reqId: string, res: express.Response) {
...
app.get('/v1/version', (_req: Request, res: Response) => res.json({ version: '1.0.0' }));

// Boards
app.post('/v1/boards', requireAuth, (req: Request, res: Response) => {
...
app.get('/v1/boards', requireAuth, (req: Request, res: Response) => {
  const requestId = (req as any).requestId as string;
  const userId = (req as any).userId as string;
  ...
  const boards = rows.map((r) => ({
    ...
    myRole: (r.my_role ?? 'reader') as any,
    membersCount: Number(r.members_count || 1)
  }));
  return res.json({ boards, nextCursor: null });
});

app.get('/v1/boards/:boardId', requireAuth, (req: Request, res: Response) => {
  const requestId = (req as any).requestId as string;
  const userId = (req as any).userId as string;
  const boardId = req.params.boardId;
  const board = db.prepare('SELECT * FROM boards WHERE id = ?').get(boardId) as any;
  if (!board) return res.status(404).json(errorEnvelope('not_found', 'Board not found', requestId));
  const role = roleFor(boardId, userId);
  if (!role) return res.status(403).json(errorEnvelope('forbidden', 'Access denied', requestId));
  const membersCount = db.prepare("SELECT COUNT(*) AS c FROM memberships WHERE board_id = ? AND status='active'").get(boardId) as any;
...
// Columns
app.post('/v1/boards/:boardId/columns', requireAuth, (req: Request, res: Response) => {
  const requestId = (req as any).requestId as string;
  const userId = (req as any).userId as string;
  ...
  if (!role) return;
  const trimmed = (name || '').trim();
  if (!trimmed) return res.status(422).json(errorEnvelope('validation_error', 'name must not be empty', requestId, { name: 'required_non_empty' }));
  const cols = db.prepare('SELECT id, sort_key FROM columns WHERE board_id = ? ORDER BY sort_key ASC, id ASC').all(boardId) as any[];
  const byId = new Map(cols.map((c) => [c.id, c]));
  let left: string | null = null;
  let right: string | null = null;
  if (beforeColumnId) {
    const r = byId.get(beforeColumnId);
    if (!r) return res.status(422).json(errorEnvelope('validation_error', 'Invalid beforeColumnId', requestId));
    right = r.sort_key;
  }
  if (afterColumnId) {
    const l = byId.get(afterColumnId);
    if (!l) return res.status(422).json(errorEnvelope('validation_error', 'Invalid afterColumnId', requestId));
    left = l.sort_key;
  }
...
app.post('/v1/boards/:boardId/columns/:columnId:move', requireAuth, (req: Request, res: Response) => {
  const requestId = (req as any).requestId as string;
  const userId = (req as any).userId as string;
  ...
  const cols = db.prepare('SELECT id, sort_key FROM columns WHERE board_id = ? ORDER BY sort_key ASC, id ASC').all(boardId) as any[];
  const byId = new Map(cols.map((c) => [c.id, c]));
  if (!byId.has(columnId)) return res.status(404).json(errorEnvelope('not_found', 'Column not found', requestId));
  let left: string | null = null;
  let right: string | null = null;
  if (beforeColumnId) {
    const r = byId.get(beforeColumnId);
  ...
  const c = db.prepare('SELECT * FROM columns WHERE id = ?').get(columnId) as any;
  return res.json({ id: c.id, boardId: c.board_id, name: c.name, sortKey: c.sort_key, createdAt: c.created_at, updatedAt: c.updated_at });
});

// Cards
app.post('/v1/boards/:boardId/columns/:columnId/cards', requireAuth, (req: Request, res: Response) => {
  const requestId = (req as any).requestId as string;
  const userId = (req as any).userId as string;
  ...
  if (!trimmed) return res.status(422).json(errorEnvelope('validation_error', 'title must not be empty', requestId, { title: 'required_non_empty' }));
  const cards = db.prepare('SELECT id, sort_key FROM cards WHERE board_id = ? AND column_id = ? ORDER BY sort_key ASC, id ASC').all(boardId, columnId) as any[];
  const byId = new Map(cards.map((c) => [c.id, c]));
  let left: string | null = null;
  let right: string | null = null;
  if (beforeCardId) {
    const r = byId.get(beforeCardId);
    if (!r) return res.status(422).json(errorEnvelope('validation_error', 'Invalid beforeCardId', requestId));
    right = r.sort_key;
  }
  if (afterCardId) {
    const l = byId.get(afterCardId);
    if (!l) return res.status(422).json(errorEnvelope('validation_error', 'Invalid afterCardId', requestId));
    left = l.sort_key;
  }
...
app.post('/v1/boards/:boardId/cards/:cardId:move', requireAuth, (req: Request, res: Response) => {
  const requestId = (req as any).requestId as string;
  const userId = (req as any).userId as string;
  ...
  if (!card) return res.status(404).json(errorEnvelope('not_found', 'Card not found', requestId));
  if (card.board_id !== boardId) return res.status(409).json(errorEnvelope('invalid_move', 'Card can be moved only within the same board.', requestId));
  if (expectedVersion != null && Number(expectedVersion) !== Number(card.version)) return res.status(412).json(errorEnvelope('precondition_failed', 'Version mismatch', requestId));
  ...
  const cards = db
    ...
    .all(boardId, targetColumnId) as any[];
  const byId = new Map(cards.map((c) => [c.id, c]));
  let left: string | null = null;
  let right: string | null = null;
  if (beforeCardId) {
  ...
  const c = db.prepare('SELECT * FROM cards WHERE id = ?').get(cardId) as any;
...
// Members
app.post('/v1/boards/:boardId/members', requireAuth, (req: Request, res: Response) => {
  const requestId = (req as any).requestId as string;
  const userId = (req as any).userId as string;
  const boardId = req.params.boardId;
  ...
  const r: 'admin' | 'writer' | 'reader' = memberRole || 'reader';
  const now = nowIso();
  const invId = uuid();
  const token = uuid().replace(/-/g, '');
  db.transaction(() => {
    db.prepare('INSERT OR REPLACE INTO memberships (board_id, user_id, role, status, invited_by, created_at, updated_at) VALUES (?,?,?,?,?,?,?)').run(
      boardId,
